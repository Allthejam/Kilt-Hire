...
{po.customerEmail} • {po.customerPhone}
      ...
      {/* EDIT OUTSOURCED ITEM MODAL */}
      {editOutsourcedItem && (() => {
        const item = editOutsourcedItem;
        const CATEGORIES = ['Kilts', 'Jackets', 'Waistcoats', 'Sporrans', 'Ghillie Brogues', 'Ties & Cravats', 'Sgian Dubh', 'Belts & Buckles', 'Fly Plaids', 'Kilt Pins', 'Hats & Caps', 'Socks & Flashes', 'Accessories'];
        return (
              ...
              <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between rounded-t-3xl z-10">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 bg-indigo-100 rounded-xl flex items-center justify-center">
                    <Edit3 className="w-4.5 h-4.5 text-indigo-700" />
                  </div>
                  <div>
                    <h2 className="font-extrabold text-slate-900 text-base leading-tight">Edit Outsourced Item</h2>
                    <p className="text-[11px] text-slate-400 font-mono">{item.id}</p>
                  </div>
                </div>
                <button onClick={() => setEditOutsourcedItem(null)} className="p-1.5 text-slate-400 hover:text-slate-700 rounded-full hover:bg-slate-100 transition cursor-pointer">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <form
                ...
                onSubmit={e => {
                  ...
                  upsertItem(updated).catch(err => console.warn('Edit save failed:', err));
                  showToast(`✅ Outsourced item "${updated.name}" updated successfully.`, 'success');
                  setEditOutsourcedItem(null);
                }}
              >
                {/* NAME */}
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    value={item.name}
                    onChange={e => setEditOutsourcedItem(prev => prev ? { ...prev, name: e.target.value } : null)}
                    className="w-full border border-slate-300 rounded-xl px-3 py-2 font-bold text-slate-900 outline-none focus:border-indigo-500"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block font-bold text-slate-600 mb-1">Demographic</label>
                    <select
                      value={item.sizeGroup}
                      onChange={e => setEditOutsourcedItem(prev => prev ? { ...prev, sizeGroup: e.target.value as 'Adult' | 'Kid' } : null)}
                      className="w-full border border-slate-300 rounded-xl px-3 py-2 font-bold text-slate-900 outline-none focus:border-indigo-500 bg-white"
                    >
                      <option>Adult</option>
                      <option>Kid</option>
                ...
                {/* TARTAN / DESCRIPTION + SIZE */}
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block font-bold text-slate-600 mb-1">Tartan / Colour / Description</label>
                    <input
                      type="text"
                      value={item.tartanOrColour}
                      onChange={e => setEditOutsourcedItem(prev => prev ? { ...prev, tartanOrColour: e.target.value } : null)}
                      className="w-full border border-slate-300 rounded-xl px-3 py-2 font-bold text-slate-900 outline-none focus:border-indigo-500"
                    />
                  </div>
                  <div>
                    <label className="block font-bold text-slate-600 mb-1">Size / Fit</label>
                    <input
                      type="text"
                      value={item.size}
                      onChange={e => setEditOutsourcedItem(prev => prev ? { ...prev, size: e.target.value } : null)}
                      className="w-full border border-slate-300 rounded-xl px-3 py-2 font-bold text-slate-900 outline-none focus:border-indigo-500"
                      placeholder="e.g. Custom Waist & Length"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200">
                  <div>
                    <label className="block font-bold text-slate-500 mb-1">Customer Hire (£)</label>
                    <input
                      type="number" min={0}
                      value={item.hireRate}
                      onChange={e => setEditOutsourcedItem(prev => prev ? { ...prev, hireRate: Number(e.target.value) } : null)}
                      className="w-full border border-amber-300 bg-amber-50 rounded-lg px-2 py-1.5 font-mono font-extrabold text-amber-900 outline-none"
                    />
                  </div>
                  <div>
                    <label className="block font-bold text-slate-500 mb-1">Deposit (£)</label>
                    <input
                      type="number" min={0}
                      value={item.depositAmount}
                      onChange={e => setEditOutsourcedItem(prev => prev ? { ...prev, depositAmount: Number(e.target.value) } : null)}
                      className="w-full border border-emerald-300 bg-emerald-50 rounded-lg px-2 py-1.5 font-mono font-extrabold text-emerald-900 outline-none"
                    />
                  </div>
                  <div>
                    <label className="block font-bold text-slate-500 mb-1">Sub-Hire Cost (£)</label>
                    <input
                      type="number" min={0}
                      value={item.outsourcedWholesaleCost || 0}
                      onChange={e => setEditOutsourcedItem(prev => prev ? { ...prev, outsourcedWholesaleCost: Number(e.target.value) } : null)}
                      className="w-full border border-slate-300 bg-white rounded-lg px-2 py-1.5 font-mono font-bold text-slate-900 outline-none"
                    />
                  </div>
                </div>
                ...
                {/* SUPPLIER */}
                <div>
                  <label className="block font-bold text-slate-600 mb-1">Preferred Supplier / Brand</label>
                  <input
                    type="text"
                    value={item.outsourcedSupplier || ''}
                    onChange={e => setEditOutsourcedItem(prev => prev ? { ...prev, outsourcedSupplier: e.target.value } : null)}
                    className="w-full border border-slate-300 rounded-xl px-3 py-2 font-bold text-slate-900 outline-none focus:border-indigo-500"
                    placeholder="e.g. Highland Scottish Supplies Ltd"
                  />
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setEditOutsourcedItem(null)}
                    className="flex-1 py-2.5 border border-slate-300 rounded-xl font-extrabold text-slate-700 hover:bg-slate-50 transition"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-xl shadow-sm transition flex items-center justify-center gap-2"
                  >
                    <Store className="w-4 h-4" /> Save Changes
                  </button>
                </div>
              </form>
            </div>
